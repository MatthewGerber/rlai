...
* `--gamma 0.9`:  Discount the reward passed backward to previous state-action pairs. This was an important part of 
  getting the agent to learn a useful policy. Without discounting -- particularly in early iterations -- the protracted
  fumbling about with poor actions would eventually be rewarded fully for a later success. By discounting prior actions,
  the agent eventually learns to focus on later actions leading to success.
...
anything useful. The training progression is shown below.
