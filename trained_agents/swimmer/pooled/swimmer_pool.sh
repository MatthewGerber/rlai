...
for n in $(seq 0 $((num_runners - 1))); do
    rlai train --random-seed "1234${n}" --agent rlai.core.agents.ParameterizedMdpAgent --gamma 1.0 --environment rlai.core.environments.openai_gym.Gym --gym-id Swimmer-v2 --render-every-nth-episode 100 --video-directory "$HOME/Desktop/swimmer_videos_${n}" --T 500 --train-function rlai.policy_gradient.monte_carlo.reinforce.improve --num-episodes 4000 --v-S rlai.state_value.function_approximation.ApproximateStateValueEstimator --feature-extractor rlai.core.environments.openai_gym.SignedCodingFeatureExtractor --function-approximation-model rlai.models.sklearn.SKLearnSGD --loss squared_error --sgd-alpha 0.0 --learning-rate constant --eta0 "${eta_values[$n]}" --policy rlai.policy_gradient.policies.continuous_action.ContinuousActionBetaDistributionPolicy --policy-feature-extractor rlai.core.environments.openai_gym.SignedCodingFeatureExtractor --alpha "${alpha_values[$n]}" --update-upon-every-visit True --num-episodes-per-checkpoint 1000 --checkpoint-path $"$HOME/Desktop/swimmer_${n}_checkpoint.pickle" --save-agent-path "$HOME/Desktop/swimmer_agent_${n}.pickle" --log INFO --training-pool-directory ~/Desktop/swimmer_pool --training-pool-count ${num_runners} --training-pool-iterate-episodes 10 --training-pool-evaluate-episodes 10 >>swimmer_pool.log 2>&1 &
